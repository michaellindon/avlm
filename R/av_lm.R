...
av.lm <- function(model, g = 1, vcov_estimator = NULL, ...) {
  ...
  if (!is.null(vcov_estimator)) {
    ...
    class(model) <- c("avlm_robust", class(model))
  } else {
...
#' Summary Method for Robust Anytime-Valid lm (avlm) Objects
#'
#' Computes a summary for an \code{avlm_robust} object, a linear model enhanced with anytime-valid inference
#' and heteroskedasticity-consistent standard errors.
#' In addition to the standard \code{lm} summary statistics, the p-values for the coefficient tests are
#' recalculated using an anytime-valid method.
#'
#' @param object An \code{avlm_robust} object generated by \code{av.lm}.
#' @param ... Additional arguments passed to or from other methods.
#'
#' @return A summary object of class \code{summary.avlm_robust} that includes updated p-values for the coefficients.
#' @export
summary.avlm_robust <- function(object, ...) {
  # First get the regular lm summary
  summ <- NextMethod()
  
  # Change the class to include summary.avlm first 
  class(summ) <- c("summary.avlm_robust", "summary.lm")
  ...
  number_of_coefficients <- ncol(X)
  h <- hatvalues(object)
  # Calculate weights based on the HC estimator type
  print(attr(object, "vcov_estimator"))
  weights <- switch(attr(object, "vcov_estimator"),
                    "HC0" = e^2,
  ...
  XV_hatX <- crossprod(X,  X * weights)   # V_hat is a p x p matrix
  ...
  std_errs <- sqrt(diag(solve(crossprod(X_star))))
  print(std_errs)
